...
function Fish(mass, x, y, hue) {
  ...
  this.hue =
    hue || Math.random() < 0.5 ? Math.random() * 0.5 : 1 - Math.random() * 0.5; // <- the hue is used for color generation and mating
  this.color = Fish.rgb2hex(Fish.hsv2rgb(this.hue, 1, 0.8)); // 1 s,v is full saturation
...
Fish.prototype = {
  ...
  swim: function (sea) {
    ...
    if (friends.length) this.shoal(friends);
    ...
    var bigger = [];
    for (var j in neighboors) {
      if (neighboors[j].mass > this.mass * 2) bigger.push(neighboors[j]);
    }

    // if any, avoid it/them
    if (bigger.length) this.avoid(bigger, 300);

    // find nearby fish that are way smaller than the this fish
    var smaller = [];
    for (var j in neighboors) {
      if (neighboors[j].mass < this.mass / 2) smaller.push(neighboors[j]);
    }

    // if any, chase and eat it/them
    if (smaller.length) this.eat(smaller);
    ...
    if (this.mature) {
      ...
      var mature = [];
      for (var j in neighboors)
        if (neighboors[j].mature) mature.push(neighboors[j]);
  ...
  avoid: function (fishList, dist) {
    ...
    if (Fish.showBehavior) this.color = 'navy';
  },
  ...
  eat: function (fishList) {
    ...
    if (Fish.showBehavior) this.color = 'firebrick';
  },
  ...
  shoal: function (fishList) {
    ...
    // compute vectors
    var separation = this.separate(fishList, this.separationRange).limit(
      this.maxforce
    );
    ...
    if (Fish.showBehavior) this.color = 'black';
  },
  ...
  mate: function (fishList, seaPopulation) {
    ...
    if (Fish.showBehavior) this.color = 'lightcoral';
  },

  // avoid boundaries of the screen
  boundaries: function (sea) {
    if (this.location.x < 50) this.applyForce(new Vector(this.maxforce * 3, 0));

    if (this.location.x > sea.width - 50)
      this.applyForce(new Vector(-this.maxforce * 3, 0));

    if (this.location.y < 50) this.applyForce(new Vector(0, this.maxforce * 3));
  ...
  wander: function (radius) {
    ...
    if (Fish.showBehavior) this.color = 'gray';
  },
  ...
  follow: function (target, arrive) {
    ...
    if (d < arrive) dest.setMag((d / arrive) * this.maxspeed);
    else dest.setMag(this.maxspeed);
  ...
  chase: function (fishList, action, force) {
    ...
    for (var i in fishList) {
      ...
      if (
        this.location.dist(fishList[i].location) <
        (this.length + fishList[i].length) / 2
      )
  ...
  draw: function (ctx) {
    ...
    if (this.energy < 5) this.color = 'black';

    if (Fish.showBehavior && this.mature) this.color = 'lightcoral';
    ...
    if (Fish.showBehavior) {
      ...
      if (this.avoidList && this.avoidList.length) {
        ...
        for (var i in this.avoidList) {
          ...
          ctx.lineTo(
            this.avoidList[i].location.x,
            this.avoidList[i].location.y
          );
        }
        ctx.stroke();
      }
      ...
      if (this.shoalList && this.shoalList.length) {
        ...
        for (var i in this.shoalList) {
          ctx.moveTo(this.location.x, this.location.y);
          ctx.lineTo(
            this.shoalList[i].location.x,
            this.shoalList[i].location.y
          );
        }
        ctx.stroke();
      }
  ...
  update: function (sea) {
    ...
    if (this.velocity.mag() < 3) this.velocity.setMag(5);
    ...
    // spend energy
    this.energy -=
      (this.acceleration.mag() * this.mass * this.age * this.velocity.mag()) /
      100;
...
Fish.interpolate = function (colorA, colorB) {
  ...
  } else interpolation = (colorA + colorB) / 2;
